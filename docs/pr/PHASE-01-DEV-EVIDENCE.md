...
- Target branch: dev
- Status: QA-001 changes addressed; READY FOR QA RETEST. The Phase is not QA PASSED.
- PR: https://github.com/BruceChun0820/pre-sales-knowledge-agent/pull/2; the exact tested head SHA is recorded in the PR description after each push to avoid a self-referential stale hash.
...
- Automated quality: 54 tests passed; Ruff lint and formatter checks passed.
...
- QA-001 regression: automated tests cover replacement and failure safety; a live Qdrant reproduction returned finance=0, healthcare=1, active=1 after metadata-only reclassification.
...
| P1-14 | 54 passing tests; Ruff lint and format checks pass |
...
| P1-17 | open PR https://github.com/BruceChun0820/pre-sales-knowledge-agent/pull/2 targets dev from feature/phase-01-ingestion-qdrant; exact tested head is maintained in the PR description after push; not merged |
